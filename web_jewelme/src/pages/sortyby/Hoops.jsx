...
export default function HoopsPage() {
  ...
  useEffect(() => {
    fetchAllProducts()
      .then((res) => {
        const hoops = res.data.filter(
          (product) => product.categoryId?.name?.toLowerCase() === "hoops"
        );
        setProducts(hoops);
      })
  ...
  return (
    <div className="font-serif min-h-screen flex flex-col">
      ...
      <main className="flex-grow bg-gradient-to-br from-indigo-50 to-white px-6 py-12">
        <h1 className="text-4xl font-extrabold text-center text-yellow-600 mb-10">Hoops</h1>
