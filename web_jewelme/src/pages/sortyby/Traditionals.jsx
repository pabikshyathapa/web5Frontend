    ...
    fetchAllProducts()
      .then((res) => {
        const traditionals = res.data.filter(
          (product) =>
            product.categoryId?.name?.toLowerCase() === "traditionals"
        );
      ...
      <footer className="bg-red-300 text-black px-8 py-16 grid grid-cols-1 md:grid-cols-4 gap-12 text-sm">
        <div>
          ...
          <p className="leading-relaxed">
            Discover exclusive offers, early access to new collections, style inspiration,
            and personalized recommendations just for you. Be the first to know about
            limited-time deals and member-only perks.
          </p>
        </div>
